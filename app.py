...
def get_engine():
    ...
    # Tạo cấu trúc kết nối an toàn với mật khẩu có ký tự đặc biệt
    url_object = URL.create(
        "postgresql",
        username=str(conf['username']).strip(),
        password=str(conf['password']).strip(), # Xử lý an toàn ký tự @ trong mật khẩu
